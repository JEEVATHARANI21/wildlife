...
export default function Moments() {
  ...
  return (
    ...
    >
      ...
      <div
        ref={pinSectionRef}
        className="sticky top-0 w-full h-[100svh] overflow-hidden flex flex-col justify-between py-6 md:py-8 select-none"
      >
        {/* Top Editorial Header & Catalogue Breadcrumb */}
        <div className="flex justify-between items-center border-b border-[#D5D0C6] pb-3 px-6 md:px-14 z-20">
        ...
        <div className="relative flex-1 w-full my-auto overflow-hidden flex items-center">
          ...
          >
            {MOMENTS.map((item) => (
              <div
                key={item.id}
                className="h-full flex items-center justify-center flex-shrink-0 px-6 md:px-12"
                style={{ width: `${100 / MOMENTS.length}%` }}
              >
                {/* Each Slide: Both Image & Description Centered Together on the Page */}
                <div className="flex flex-col lg:flex-row items-center justify-center gap-8 lg:gap-14 w-full max-w-5xl mx-auto">
                  {/* Left Column: Photograph */}
                  <div className="w-full lg:w-[46%] flex items-center justify-center flex-shrink-0">
                    <div className="relative w-[280px] sm:w-[350px] md:w-[410px] aspect-[4/5] bg-[#0E0F0D] rounded-sm overflow-hidden shadow-2xl border border-black/10">
                      <img
                        src={item.src}
                        alt={item.title}
                  ...
                  <div className="w-full lg:w-[54%] flex flex-col justify-center flex-shrink-0 max-w-md lg:max-w-lg text-left">
                    ...
                    <p className="font-sans text-[11px] tracking-[0.25em] uppercase font-semibold text-[#3A3B36] mb-3">
                      {item.category}
                    </p>

                    <p className="font-sans text-xs md:text-sm text-[#4A4B45] font-light leading-relaxed mb-4">
                      {item.description}
                    </p>

                    <p className="font-sans text-[10px] tracking-[0.2em] uppercase text-[#888] mb-5">
                    ...
                    <div>
                      <a
                        href="#contact"
                        className="inline-block px-6 py-2.5 bg-[#1C1D1A] text-[#F1EFE8] font-sans text-[10px] tracking-[0.25em] uppercase hover:bg-[#B3874B] transition-colors duration-300 shadow-md"
                      >
                        Explore Catalogues
                      </a>
                    </div>
        ...
        <div className="flex justify-between items-center pt-3 border-t border-[#D5D0C6] px-6 md:px-14 z-20">
          <div className="flex items-center gap-2">
            {MOMENTS.map((m, i) => (
              <button
                key={m.id}
                onClick={() => scrollToSlide(i)}
                className="py-2 focus:outline-none group cursor-pointer"
                title={`Go to ${m.title}`}
              >
                <div
                  className={`h-[3px] rounded-full transition-all duration-500 ${
                    i === activeIdx
                      ? 'w-10 bg-[#B3874B]'
                      : 'w-3 bg-[#D5D0C6] group-hover:bg-[#999]'
                  }`}
                />
              </button>
            ))}
          ...
          <span className="font-serif italic text-xs text-[#777] truncate max-w-[280px] md:max-w-none text-right">
            "{current.title}"
          </span>
        </div>
      </div>
